...
class SecureServer(threading.Thread):
    ...
    def run(self):
        ...
        print("Listening on <%s%s:%d%s>\n" % (Fore.BLUE, self.sock.getsockname()[0], self.sock.getsockname()[1], Fore.RESET))        
        self.connection, address = self.sock.accept()

        print("Incoming connection from <%s%s:%s%s>" % (Fore.BLUE, str(address[0]), str(address[1]), Fore.RESET))
        ...
        domain = self.receive()
        self.send(domain)
        print("     agreed on domain-parameter: %sSUCCESSFUL%s" % (Fore.GREEN, Fore.RESET))
        ...
        print("     established shared_key: %sSUCCESSFUL%s\n" % (Fore.GREEN, Fore.RESET))
        ...
        while True:
            msg = self.receive()            
            msg = json.loads(msg)
            print("%s%s%s@%s%s:%s%s: %s" % (Fore.RED, msg['author'], Fore.RESET, Fore.BLUE, str(address[0]), str(address[1]), Fore.RESET, msg['quote']))
            print("%s%s%s@%s%s:%s%s: %s" % (Fore.RED, msg['author'], Fore.RESET, Fore.BLUE, str(address[0]), str(address[1]), Fore.RESET, keyAgreement.decrypt(msg['quote'])))

            msg = quote.get()
            msg['author'] = msg['author'].replace(' ', '')
            msg['quote'] = keyAgreement.encrypt(msg['quote'])
            self.send(json.dumps(msg))
            print("%s%s%s@%s%s:%s%s: %s" % (Fore.RED, msg['author'], Fore.RESET, Fore.BLUE, self.sock.getsockname()[0], self.sock.getsockname()[1], Fore.RESET, keyAgreement.decrypt(msg['quote'])))
            print("%s%s%s@%s%s:%s%s: %s" % (Fore.RED, msg['author'], Fore.RESET, Fore.BLUE, self.sock.getsockname()[0], self.sock.getsockname()[1], Fore.RESET, msg['quote']))
