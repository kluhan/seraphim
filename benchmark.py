        ...
        def run(self):
                for size in self.sizes:
                        ...
                        for algorithm in self.algorithms:
                                bar = Bar(algorithm[1].replace("@", "@"+str(size)),  suffix='%(percent).1f%% - ETA: %(eta)ds - AVG: %(avg).5fs - ELA: %(elapsed)ds', max=self.loop_count)
                                t = time.process_time()
                        ...
                        for algo_time in times:
                                factor = "{:.2f}".format(times[self.reference_algorithm][1]/algo_time[1])
